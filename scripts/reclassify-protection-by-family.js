/**
 * Reclasse les produits de la gamme "Accessoires > Protection" (Coques + Housses et Étuis, ~695
 * produits) par FAMILLE D'APPAREIL (iPhone, Samsung, Xiaomi, Huawei/Honor, iPad, Mac...), exactement
 * comme scripts/reclassify-protection-ecran-by-family.js l'a fait pour "Protection Écran".
 *
 * Avant : 2 modèles ("Coques" ~522 produits, "Housses et Étuis" ~172 produits) mélangeant toutes les
 * marques — impossible de filtrer par appareil depuis le menu ou l'URL.
 * Après : un modèle par famille d'appareil (Coques et Housses confondues), filtrable normalement.
 *
 * Comme pour Protection Écran, le classement se fait par FAMILLE (ex: "iPhone") et non par modèle
 * précis (ex: "iPhone 15"), car certains titres couvrent plusieurs modèles à la fois et un produit
 * ne peut être rattaché qu'à un seul modèle sur le site — la famille reste toujours exacte.
 *
 * Idempotent : peut être relancé sans risque. Les anciens modèles ("Coques", "Housses et Étuis")
 * sont supprimés à la fin s'ils sont vides.
 *
 * MODE APERÇU (par défaut, aucune écriture) :
 *   node scripts/reclassify-protection-by-family.js
 *
 * MODE RÉEL :
 *   node scripts/reclassify-protection-by-family.js --apply
 */
...
async function main() {
  ...
  const line = await prisma.productLine.findFirst({ where: { brandId: brand.id, slug: 'protection' } });
  if (!line) {
    console.error('Gamme "Protection" introuvable sous "Accessoires".');
    process.exit(1);
  }
  ...
  console.log(
    `${products.length} produit(s) trouvé(s) dans "Protection" (actuellement répartis sur ${oldModels.length} modèle(s) : ${oldModels
      .map((m) => `"${m.name}"`)
      .join(', ')}).\n`
  );
