...
const schema = z.object({
  ...
  carrier: z.enum(['CHRONOPOST', 'COLISSIMO']),
});

const CARRIER_LABELS: Record<string, string> = {
  CHRONOPOST: 'Chronopost',
  COLISSIMO: 'Colissimo recommandé',
};
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const previousCarrier = repair.inboundCarrier;

  await prisma.mailInRepair.update({
    where: { id: params.id },
    data: {
      inboundTrackingNumber: parsed.data.trackingNumber,
      inboundCarrier: parsed.data.carrier,
    },
  });

  // On notifie l'admin uniquement si c'est un nouveau numéro ou un changement de transporteur (pas à
  // chaque simple correction mineure répétée), pour ne pas noyer la boîte mail.
  if (previousTracking !== parsed.data.trackingNumber || previousCarrier !== parsed.data.carrier) {
    const carrierLabel = CARRIER_LABELS[parsed.data.carrier] ?? parsed.data.carrier;
    ...
    if (resend) {
      try {
        await resend.emails.send({
          from: FROM,
          to: ADMIN_EMAIL,
          subject: `📦 Suivi ${carrierLabel} renseigné par ${repair.customerName} — ${repair.deviceBrand} ${repair.deviceModel}`,
          html: `
            <div style="font-family: Arial, Helvetica, sans-serif; max-width: 560px; margin: 0 auto;">
              ...
              <div style="background:#ffffff; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px; padding: 24px;">
                <p style="color:#374151; font-size: 14px; line-height: 1.6;">
                  ...
                  de suivi de son envoi ${carrierLabel} :
                </p>
